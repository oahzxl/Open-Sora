...
def collate_function(batch: List[Tuple[List[int], List[int]]], pad_token_id: int) -> Dict[str, torch.Tensor]:
    ...
    padded_token_ids = [
        token_ids + [pad_token_id for _ in range(0, max_length - len(token_ids))] for token_ids, _ in batch
    ]
...
def masking_function(
    ...
    if rand_or_unmask_prob > 0:
        ...
        if rand_mask is not None:
            ...
            tokenized_ids[rand_mask] = np.random.choice(tokenizer.vocab_size, num_rand, p=probs)
...
def create_data_iterator(
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn_partial)
...
class RobertaLMHeadWithMaskedPredict(RobertaLMHead):
    def __init__(self, config: RobertaConfig, embedding_weight: Optional[torch.Tensor] = None) -> None:
    ...
    def forward(  # pylint: disable=arguments-differ
        ...
        **kwargs,
        ...
        if masked_token_indices is not None:
            features = torch.index_select(features.view(-1, features.shape[-1]), 0, masked_token_indices)
...
class RobertaMLMModel(RobertaPreTrainedModel):
    def __init__(self, config: RobertaConfig, encoder: RobertaModel) -> None:
        ...
        self.lm_head = RobertaLMHeadWithMaskedPredict(config, self.encoder.embeddings.word_embeddings.weight)
    ...
    def forward(
        ...
        sequence_output, *_ = self.encoder(input_ids=src_tokens, attention_mask=attention_mask, return_dict=False)

        pad_token_id = self.config.pad_token_id
        # (labels have also been padded with pad_token_id)
        # filter out all masked labels
        # shape: (num_masked_tokens,)
        masked_token_indexes = torch.nonzero((tgt_tokens != pad_token_id).view(-1)).view(-1)
...
def load_model_checkpoint(
    ...
    """
    log_dist(f"Loading model and optimizer checkpoint from {load_checkpoint_dir}", ranks=[0], level=logging.INFO)
    ...
    checkpoint_files = sorted(
        checkpoint_files,
        key=lambda path: int(re.search(r"iter_(?P<iter_no>\d+)\.pt", path.name).group("iter_no")),
    )
    latest_checkpoint_path = checkpoint_files[-1]
    checkpoint_step = int(re.search(r"iter_(?P<iter_no>\d+)\.pt", latest_checkpoint_path.name).group("iter_no"))
...
def train(
    ...
    log_dist(f"Experiment Directory created at {exp_dir}", ranks=[0], level=logging.INFO)
    ...
    for step, batch in enumerate(data_iterator, start=start_step):
        ...
        if step % log_every == 0:
            log_dist("Loss: {0:.4f}".format(np.mean(losses)), ranks=[0], level=logging.INFO)
            if is_rank_0():
